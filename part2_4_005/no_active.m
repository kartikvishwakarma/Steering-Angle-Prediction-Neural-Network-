...
clear all;
%path = 'F:\steering';
path='/home/clab2/workspace/data/steering';
data='data.txt';
...
v_x=[];
v_y=[];
file=fopen('test_005_error.txt','a');
...
%--------------------validation_model-------------------------------%    
    ...
    for vk=1:batch_size:floor(valid_size/batch_size)*batch_size
        ...
        v_x1=1./(1.+exp(-w1*[0.001*ones(1,batch_size); v_x]));
        
        
       
        
        v_x2=1./(1.+exp(-w2*[0.001*ones(1,batch_size); v_x1]));
        
        
        v_x3=w3*[0.001*ones(1,batch_size);v_x2];
        E=(v_x3-v_y).*(v_x3-v_y);
...
%--------------------train_model------------------------------------%    
    ...
    for k=1:batch_size:floor(train_size/batch_size)*batch_size
        ...
        x1=1./(1.+exp(-w1*[0.001*ones(1,batch_size); x]));
        x2=1./(1.+exp(-w2*[0.001*ones(1,batch_size); x1]));
        x3=  (w3* [0.001*ones(1,batch_size);x2]);       %1x64
        
        delta2=(x3-y);
        dE3=delta2*[0.001*ones(1,batch_size);x2]';
        
        delta1 = (w3(:,2:end)'*delta2).*(x2.*(1-x2));
        %delta1 = (w3(:,2:end)'*delta2);
        dE2 = delta1*[0.001*ones(1,batch_size);x1]';
        
        
        delta0 = (w2(:,2:end)'*delta1).*(x1.*(1-x1));
        %delta0 = (w2(:,2:end)'*delta1);
        dE1 = delta0*[0.001*ones(1,batch_size);x]';
